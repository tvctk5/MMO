...
{
    [ModuleInfo(Name = "MO : Currency", Code = "MCurrency", Order = 50)]
    public class MCurrencyController : Controller
    {
        ...
        public void ActionIndex(MCurrencyModel model)
        {
        ...
        {
            var item = ModCurrencyService.Instance.CreateQuery()
                            .Where(o => o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModCurrencyService.Instance.CreateQuery()
